...
exports.getAllPlaces = async (req, res, next) => {
  ...
  if (!allPlaces || allPlaces.length === 0) return next(new HttpError("No places cound be found", 404))
...
exports.getPlaceByPlaceId = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError("Could not find place with this ID, Code 1", 404))
  }

  if (!foundPlace) return next(new HttpError("Could not find place with this ID, Code 2"))
...
exports.getPlacesByUserId = async (req, res, next) => {
  ...
  try {
    ...
    // foundPlaceByUserId = await PlaceModel.find({ creator: userId })
    console.log(foundPlacesByUserId);
  } catch (error) {
    return next(new HttpError("Could not find place with this USER ID, Code 1", 404))
  }
  ...
  res.json({ userPlaces: foundPlacesByUserId })
}
...
exports.createPlace = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) return next(new HttpError("invalid input fields", 422))
...
exports.updatePlace = async (req, res, next) => {
  ...
  const { placeId } = req.params;
  if (!mongoose.isValidObjectId(placeId)) return next(new HttpError("Not valid ID", 422))
  ...
  try {
    ...
    return next(new HttpError("Could not find place with this ID, Code 1", 500))
  }

  if (!foundPlace || foundPlace.length === 0) {
    return next(new HttpError("Could not find place with this ID, Code 2", 404))
  }
...
exports.deletePlace = async (req, res, next) => {
  ...
  if (!foundPlace) return next(new HttpError("Could not find place with this ID", 404))
