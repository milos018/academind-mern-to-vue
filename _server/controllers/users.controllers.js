...
exports.getUsers = async (req, res, next) => {
  ...
  if (!users || users.length === 0) return next(new HttpError("No users found", 404))
...
exports.signup = async (req, res, next) => {
  ...
  if (existingUser) return next(new HttpError("Could not create User with this email", 401))
...
exports.login = async (req, res, next) => {
  ...
  if (!passwordChecked) return next(new HttpError("Could not authorize user. Code 2", 422))
